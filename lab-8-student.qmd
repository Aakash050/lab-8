...
table <- type_of(surveys) 

type_table <- table |>
  tibble(name = names(table),
       type = table)

final_table <- type_table |>
...
evals |>
  bind_cols(
    evals |>
    map_at(
      .at=c("teacher_id", "weekday", "academic_degree", "seniority", "sex"),
      .f=as.factor
    )
  )
```
...
evals_table <- evals |>
  mutate(sen_level=if_else(seniority<=4,
                           "Junior",
                           "Senior"),
         #Found format for case_when in help section of r-studio
         academic_degree_full=case_when(
           academic_degree == "ma" ~ "Masters",
           academic_degree == "dr" ~ "Doctor",
           academic_degree == "prof" ~ "Professor",
           academic_degree == "no_dgr" ~ "No Degree")) |>
  select(sex, 
         sen_level, 
         academic_degree_full,
         teacher_id)|>
  summarize(
    female_count=n_distinct(teacher_id[sex=="female"]),
    male_count=n_distinct(teacher_id[sex=="male"]),
    Junior=n_distinct(teacher_id[sen_level=="Junior"]),
    Senior=n_distinct(teacher_id[sen_level=="Senior"]),
    No_Degree=n_distinct(teacher_id[academic_degree_full=="No Degree"]),
    Masters=n_distinct(teacher_id[academic_degree_full=="Masters"]),
    Doctor=n_distinct(teacher_id[academic_degree_full=="Doctor"]),
    Professor=n_distinct(teacher_id[academic_degree_full=="Professor"]))
final_table <- evals_table |>
